...
int main(int argc, char **argv) {
    ...
    std::string topic_joint_trajectory_publisher, topic_action_trajectory_publisher, compliance_trajectory_action_name, topic_joint_group_vel_controller_publisher;

    // std::vector<double> mass_model_matrix, damping_model_matrix;
    std::vector<double> mass_model_matrix, damping_model_matrix, spring_model_matrix;
    ...
    // ---- LOADING compliance trajectory action name ---- //
    if (!nh.param<std::string>("admittance_controller_Node/topic_compliance_trajectory_action", compliance_trajectory_action_name, "compliance_trajectory")) {ROS_ERROR("Couldn't retrieve the compliance_trajectory's name.");}
    ...
    // ---- add a spring matrix---- //
    if (!nh.getParam("admittance_controller_Node/spring_matrix", spring_model_matrix)) {ROS_ERROR("Couldn't retrieve the Spring Matrix parameter.");}
    ...
    admittance_control *ac = new admittance_control(
        ...
        compliance_trajectory_action_name,
        mass_model_matrix, damping_model_matrix, spring_model_matrix,
        force_dead_zone, torque_dead_zone, admittance_weight,
        joint_limits, maximum_velocity, maximum_acceleration);
    
    ac -> switch_controller_to_scaled_pose();  //not in compliance mode by default
    while (ros::ok()) {
        ac->spinner();
    }
